...
import { Calendar, Button, Layout } from "antd";
...
const CalendarView = () => {
  ...
  const filterListDate = (value) => {
    return slots.filter(
      (item) =>
        Number(item.slotDate.split("-")[1]) === value.date() &&
        item.slotDate.slice(0, 2) === displayDate.slice(0, 2) &&
        item.slotDate.slice(6) === displayDate.slice(6)
    );
  };
  ...
  const filterListMonth = (value) => {
    return slots.filter(
      (item) =>
        Number(item.slotDate.split("-")[0]) === value.month() + 1 &&
        item.slotDate.slice(6) === displayDate.slice(6)
    );
  };
  ...
  const monthCellRender = (value) => {
    ...
    if (data.length > 0) {
      return <div>{`${data.length} appointments total`}</div>;
    }
  };
